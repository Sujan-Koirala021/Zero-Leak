    ...
    <header className={`top-0 w-full z-50 transition-all duration-300 ease-in-out min-h-16 flex justify-center md:px-24 ${scrolled ? 'py-5 bg-white' : 'py-8 bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        <div className="flex items-center">
          <img src={Logo} alt="Logo Img" className='w-12 h-10 mr-3' />
          <Link to='/'><span className="text-xl font-bold tracking-tight text-gray-800">Zero<span className="text-red-600">Leak</span></span></Link>
        </div>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <Link to='/explore_kb' className="text-md font-medium text-gray-700 hover:text-red-600 transition-colors">Explore</Link>
          <Link to='/chat_agent' className="text-md font-medium text-gray-700 hover:text-red-600 transition-colors">Chat Now</Link>
        </nav>
      ...
      <div className={`absolute inset-0 top-[4.45rem] bg-white z-40 transform transition-transform duration-300 ease-in-out md:hidden ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="container mx-auto px-4 py-10 flex flex-col gap-6 bg-white min-h-screen">
          ...
          <Link to='/faq' className="text-lg font-medium p-3 hover:bg-red-50 rounded-lg text-gray-700 hover:text-red-600 transition-colors" onClick={() => setMobileMenuOpen(false)}>FAQ</Link>
          <Link to='/screen' className="text-lg font-medium p-3 hover:bg-red-50 rounded-lg text-gray-700 hover:text-red-600 transition-colors" onClick={() => setMobileMenuOpen(false)}>Screen</Link>
